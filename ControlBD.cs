...
namespace SGVentas
{
    class ControlBD 
    {
        ...
        public DataTable ConsultarUsuarios()
        {
            ...
            {
                cn.Open();
                SQLiteDataAdapter da = new SQLiteDataAdapter("SELECT cod_usuario, correo_usuario, t.cod_tipousuario as COD_TIPOUSUARIO, " +
                    "t.nombre_tipousuario as NOM_TIPOUSUARIO, u.cod_empleado as COD_EMPLEADO, e.nombre_empleado as NOM_EMPLEADO, " +
                    "e.apellido_empleado as APE_EMPLEADO FROM (((empleado AS e INNER JOIN cargo AS c ON e.cod_cargo = c.cod_cargo) INNER JOIN " +
                    "area AS a ON e.cod_area = a.cod_area) INNER JOIN USUARIO AS u ON e.COD_EMPLEADO = u.COD_EMPLEADO) INNER JOIN TIPO_USUARIO AS t " +
                    "ON t.cod_tipousuario = u.cod_tipousuario WHERE NOT estado_usuario = 'O'", cn);
                da.Fill(dt);
            }
            catch (SQLiteException ex)
            {
        ...
        public DataTable BuscarUsuario(string clave)
        {
            ...
            {
                cn.Open();
                SQLiteCommand comando = new SQLiteCommand("SELECT cod_usuario, correo_usuario, t.cod_tipousuario as COD_TIPOUSUARIO, " +
                    "t.nombre_tipousuario as NOM_TIPOUSUARIO, u.cod_empleado as COD_EMPLEADO, e.nombre_empleado as NOM_EMPLEADO, " +
                    "e.apellido_empleado as APE_EMPLEADO FROM (usuario as u INNER JOIN tipo_usuario AS t ON u.cod_tipousuario = t.cod_tipousuario) " +
                    "INNER JOIN empleado AS e ON u.cod_empleado = e.cod_empleado WHERE NOT estado_usuario = 'O' AND " +
                    "(correo_usuario LIKE @clave OR nom_empleado LIKE @clave OR ape_empleado LIKE @clave);", cn);
        ...
        public DataTable BuscarInve(string nombreInv)
        {
            SQLiteDataAdapter adapter = new SQLiteDataAdapter();
            try
            {
